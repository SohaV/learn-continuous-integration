...
1. What does the **runs-on** string
   ...
   The runs-on string specifies the type of machine or runner on which the GitHub Actions job will execute. In this case, "runs-on: self-hosted" means the job will run on a self-hosted runner.

2. In `main.yml`, on which branch do the jest tests run when a push to main branch is made?

   When a push is made to the main branch, the Jest tests run on the main branch because of the following configuration -

   on:
   push:
   branches: - main

   This triggers the workflow to execute the tests for the main branch.

3. In `main.yml`, on which branch do the jest tests run when a pull request is submitted to the deploy branch?

   When a pull request is submitted to the deploy branch, the Jest tests run on the source branch of the pull request. In this case, it is the main branch.
